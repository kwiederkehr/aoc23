        ...
        for col, char in enumerate(line):
            if char.isdigit():
                ...
                if col + 1 == len(line) or not line[col + 1].isdigit():
                    numbers_with_coordinates.add(NumberWithCoordinates(value=int(num_as_str), row=row, col_start=first_col_for_num_as_str, col_end=col))
                    first_col_for_num_as_str = None
                    num_as_str=""

    return numbers_with_coordinates

def get_coords_to_check(number: NumberWithCoordinates) -> set[Coordinate]:
    coords_to_check = set()
    # the row before
    for i in range(number.col_start-1, number.col_end+2):
...
def has_symbol_near_coords(number: NumberWithCoordinates, symbols:set[Coordinate]) -> bool:
    coords_to_check = get_coords_to_check(number)
