...
def check_if_a_number_starts_here_and_replace_if_yes(input_string: str, idx: int) -> str:
    numbers = [
        ("one", "1"),
        ("two", "2"),
        ("three", "3"),
    ...
    for number in numbers:
        if input_string[idx:len(number[0])+idx] == number[0]:
            modified_input_string = input_string.replace(number[0], number[1], 1)
            return modified_input_string

    return input_string


def convert_any_spelled_numbers_into_numbers(input_string) -> str:

    # go through the input string from the beginning
    # see something that looks like a number string? check if it is,
    # then replace
    ...
    while idx < len(input_string):
        for number in numbers:
            if input_string[idx:len(number[0]) + idx] == number[0]:
                input_string = f"{input_string[:idx]}{number[1]}{input_string[idx:]}"
                idx += 1
                continue
        idx += 1
        #
        #
        # modified_input_string = check_if_a_number_starts_here_and_replace_if_yes(input_string, idx)
        # if not modified_input_string == input_string:
        #     # start over, easier than keeping track of what the next idx is
        #     # possibly inefficient but looking at input we look ok
...
def first_and_last_number_from_string_as_2_digit_number(input_string: str) -> int:
    input_string = convert_any_spelled_numbers_into_numbers(input_string)

    # iterate through the string from the front and the back until the first and last number
    forward_idx = 0
    backward_idx = len(input_string) - 1
    first_int_as_char = None
    last_int_as_char = None

    while forward_idx <= backward_idx:
        if input_string[forward_idx].isdigit():
            first_int_as_char = input_string[forward_idx]
        else:
            forward_idx += 1

        if input_string[backward_idx].isdigit():
            last_int_as_char = input_string[backward_idx]
        else:
            backward_idx -= 1

        if first_int_as_char is not None and last_int_as_char is not None:
            break

    return int(first_int_as_char+last_int_as_char)


def main():
    numbers_from_strings: list[int] = [first_and_last_number_from_string_as_2_digit_number(input_string) for input_string in input_strings]
    total = 0
    for number_from_string in numbers_from_strings:
        total += number_from_string
